...
class PerfilActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnCerrarSesion.setOnClickListener {
            GlobalScope.launch {
                val registro = Registro("0","Usuario cerro sesión", utils.getCurrentDate(), LocationService.lat0, LocationService.long0, preferencias.getGlobalData(this@PerfilActivity, "TM"))
                //Aqui iba el mismo codigo de sendButtonData()
                utils.doRequest(registro, this@PerfilActivity)
            }
